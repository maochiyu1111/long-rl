    ...
    for index, batch_idx in enumerate(batch_indices):
        ...
        with torch.inference_mode():
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                if dist.get_rank() == 0:
                    v = video.detach()
                    print("[VAE decode] shape:", tuple(v.shape))
                    print("[VAE decode] dtype:", v.dtype)
                    print("[VAE decode] device:", v.device)
                    print("[VAE decode] min/max:", float(v.min()), float(v.max()))
                    print("[VAE decode] mean/std:", float(v.mean()), float(v.std()))
...
def train_one_step(
    ...
    for i,sample in list(enumerate(samples_batched_list)):
        ...
        if dist.get_rank()%8==0:
            with open(f'./loss_log_{dist.get_rank()}.txt', 'a') as f:
                print("vq loss/mq loss", vq_loss.item(), mq_loss.item(), file=f)
                print("vq/mq reward", sample["vq_rewards"].item(), sample["mq_rewards"].item(), file=f)
                print("ratio", ratio.item() if torch.is_tensor(ratio) else ratio, file=f)
                print("vq advantage/mq advantage", vq_advantages.item(), mq_advantages.item(), file=f)
                print("final loss", final_loss.item(), file=f)
                print("-" * 50, file=f)
        dist.barrier()

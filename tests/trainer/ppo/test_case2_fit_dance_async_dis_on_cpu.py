...
class _RecordingAsyncActorWG:
    ...
    def update_actor_dance_async(self, batch: DataProto):
        ...
        return _ImmediateFuture(DataProto(meta_info={"metrics": {"actor/loss": 0.25}}))


class _RecordingAsyncRolloutRefWG:
    def __init__(self):
        self.world_size = 2
        self.generate_inputs = []
...
def test_fit_dis_dance_case2_uses_async_dual_rollout_encoder_state_protocol(monkeypatch):
    monkeypatch.setattr(
    ...
    trainer.actor_wg = _RecordingAsyncActorWG()
    trainer.rollout_ref_wg = _RecordingAsyncRolloutRefWG()
    ...
    assert len(trainer.actor_wg.generate_inputs) == 1
    assert len(trainer.rollout_ref_wg.generate_inputs) == 1
    actor_generate_input = trainer.actor_wg.generate_inputs[0]
    rollout_generate_input = trainer.rollout_ref_wg.generate_inputs[0]
    assert set(actor_generate_input.batch.keys()) >= {"encoder_hidden_states", "encoder_attention_mask", "seed"}
    assert set(rollout_generate_input.batch.keys()) >= {"encoder_hidden_states", "encoder_attention_mask", "seed"}
    assert actor_generate_input.meta_info["dispatch_role"] == "actor"
    assert rollout_generate_input.meta_info["dispatch_role"] == "rollout_ref"
    assert len(trainer.actor_wg.update_inputs) >= 1
    update_input = trainer.actor_wg.update_inputs[0]
    assert "vq_advantages" in update_input.batch.keys()
    assert "mq_advantages" in update_input.batch.keys()
    assert update_input.meta_info["use_precomputed_advantages"] is True
    assert update_input.meta_info["skip_bestofn"] is True


def test_fit_dis_dance_case2_fail_fast_reports_disaggregate_preconditions():
    trainer = RayPPOTrainer.__new__(RayPPOTrainer)
    trainer.diffusion = True
...
def test_create_dataloader_dance_case2_skips_validation_construction(monkeypatch):
    created_dataloaders = []
